          ...
          <tbody>{rows.map((row) => {
            ...
            return (
              <tr key={row.id} className={invalid ? 'bg-rose-50 dark:bg-rose-950/20' : ''}>
                <td className="border-b border-zinc-100 py-3 pr-3 align-top dark:border-zinc-900"><input className={`w-full rounded-md border px-3 py-2 uppercase dark:bg-zinc-900 ${invalid || duplicate ? 'border-rose-400' : 'border-zinc-300 dark:border-zinc-700'}`} value={row.symbol} onChange={(event) => { const nextSymbol = normalizeUserSymbol(event.target.value); updateRow(row.id, { symbol: nextSymbol, market: row.symbol === '' ? inferMarket(nextSymbol) : row.market }); }} placeholder="THYAO" />{duplicate ? <p className="mt-1 text-xs text-rose-600">Tekrar eden sembol; hesapta birleştirilir.</p> : null}{priceErrors[key] ? <p className="mt-1 text-xs text-amber-600">{priceErrors[key]}</p> : null}</td>
